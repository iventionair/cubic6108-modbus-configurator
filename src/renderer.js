...
function getWifiSSID() {
  ...
  parser.on("data", (data) => {
    ...
    port.flush();
    port.close();
    document.getElementById("ValueWifiSSID").innerHTML = data
      .toString()
      .replace("+ok=", "");
    getWifiPassword();
  });
...
function getWifiPassword() {
  ...
  parser.on("data", (data) => {
    ...
    port.flush();
    port.close();
    document.getElementById("ValueWifiPASSWD").innerHTML = data
      .toString()
      .replace("+ok=", "");
    getServer();
  });
...
function getServer() {
  ...
  parser.on("data", (data) => {
    ...
    port.flush();
    port.close();
    document.getElementById("ValueServer").innerHTML = data
      .toString()
      .replace("+ok=", "");
    getSensorData();
  });
...
function setServer() {
  ...
  port.write(
    "XFAT+NETP=TCP,CLIENT," + serverPort + "," + serverIP + "\r\n",
    function (err) {
      errorSerialPort(err);
      console.log("message written");
    }
  ...
  var CRCMaster = {
    ...
    Calculate: function (str, inputType) {
      ...
      if (this.CleanString(inputType)) {
        crcinputcrc16modbus = this.CRC16Modbus()
          .toString(16)
          .toUpperCase()
          .padStart(4, "0");
        crcinputcrc16modbus =
          crcinputcrc16modbus.substr(2) + crcinputcrc16modbus.substr(0, 2); //swap bytes
      }
    },
  };
  CRCMaster.init();
